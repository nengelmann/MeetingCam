...
# Code from luxonis/depthai-python github repository:
# https://github.com/luxonis/depthai-python/blob/main/examples/ColorCamera/rgb_uvc.py
...
parser.add_argument(
    "-fb", "--flash-bootloader", default=False, action="store_true"
)
parser.add_argument("-f", "--flash-app", default=False, action="store_true")
parser.add_argument(
    "-l", "--load-and-exit", default=False, action="store_true"
)
...
def flash(pipeline=None):
    ...
    def progress(p):
        return print(f"Flashing progress: {p * 100:.1f}%")
...
if args.load_and_exit:
    ...
    print(
        "To reconnect with depthai, a device power-cycle may be required in"
        " some cases"
    )
